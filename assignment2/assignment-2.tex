...
\begin{document}
	...
	%
	\author{ Devananth V- EP20BTECH11004 %<-this  stops a space
...
\end{align}
\solution Download the python code for the above problem by running the given commmand
\begin{lstlisting}
	$ wget https://github.com/ep20btech11004/lssp/blob/main/assignment2/codes/q_1.py
\end{lstlisting}
\end{enumerate}
...
\begin{lstlisting}
	$ wget https://github.com/ep20btech11004/lssp/blob/main/assignment2/codes/q_2.2.py
\end{lstlisting}
\begin{figure}[!ht]
	\centering
	\includegraphics[width=\columnwidth]{./figs/q_2-2.png}
...
\begin{lstlisting}
	$ wget https://github.com/ep20btech11004/lssp/blob/main/assignment2/codes/q_2.5.py
\end{lstlisting}
\begin{figure}[!ht]
	\centering
	\includegraphics[width=\columnwidth]{./figs/q_2-5.png}
	\caption{The stem plot of $y(n)$}
\end{figure}
%problem 2.6
\item Find $Y^{+}(z)$. 
\solution Take the one-sided $Z$-transform on the both sides of the eqn \eqref{eq:10-orig-diff-rev},
\begin{align}
	&\mathcal{Z}^+\sbrak{y(n)} = \mathcal{Z}^+\sbrak{x(n + 1)} + \mathcal{Z}^+\sbrak{x(n - 1)} \\
	&Y^+(z) = zX^+(z) - zx(0) + z^{-1}X^+(z) + zx(-1) 
\end{align}
On substituting the values of $X^+(z)$, $x(0)$, $x(1)$ and $x(-1)$. 
[here, $x(-1) = 0$, as $x(n) = 0$ $\forall n < 0$]
\begin{align}
...
\item Find $y(n)$.
\label{pr:1-3}
\solution As $Y^+(z) = (1 + 2z^{-1})X^+(z)$, hence using the \eqref{eq:one-Z},
\begin{align}
	Y^+(z) &= (1 + 2z^{-1})\sum_{n = 0}^{\infty}x(n)z^{-n} \\
	&= \sum_{n = 0}^{\infty}x(n)z^{-n} + \sum_{n = 1}^{\infty}2x(n - 1)z^{-n} \\
	&= x(0) + \sum_{n = 1}^{\infty}\brak{x(n) + 2x(n - 1)}z^{-n}
\end{align}
Now, as $y(0) = x(0) = 1$ and using the fact that $\alpha$ and 
$\beta$ are the roots of the eqn $z^2 - z - 1 = 0$,
\begin{align}
